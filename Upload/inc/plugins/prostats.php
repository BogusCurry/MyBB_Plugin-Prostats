...
function prostats_run_global()
{
	...
	if (isset($GLOBALS['templatelist']))
	{
		if ($mybb->settings['ps_enable'] && defined('THIS_SCRIPT'))
		{
			...
			{
				if (($mybb->settings['ps_index'] && THIS_SCRIPT == 'index.php') || ($mybb->settings['ps_portal'] && THIS_SCRIPT == 'portal.php') || $mybb->settings['ps_global_tag'])
				{
...
function prostats_run_index($force = false)
{
	...
	if (ceil($mybb->settings['ps_num_rows']) != $mybb->settings['ps_num_rows'] || ceil($mybb->settings['ps_subject_length']) != $mybb->settings['ps_subject_length'])
	{
		return false;
	}

	if (intval($mybb->settings['ps_num_rows']) < 3)
	{
		return false;
	}
	...
	if (!$mybb->settings['ps_index'] && !$force)
	{
		return false;
	}
	...
	if ($mybb->settings['ps_position'] == 1)
	{
...
function ps_GetExtraData($cellnum, $fullrows = false)
{
	...
	else
	{
		...
		$rows = (ceil($rows / 2) - 1);
		if (!(($mybb->settings['ps_num_rows']) % 2) && !($cellnum % 2))
		{
			++$rows;
		}
	}
	...
	case 1:
		$res = ps_GetMostReplies($rows);
		break;

	case 2:
		$res = ps_GetMostReputation($rows);
		break;

	case 3:
		$res = ps_GetMostThanks($rows);
		break;

	case 4:
		$res = ps_GetMostViewed($rows);
		break;

	case 5:
		$res = ps_GetNewMembers($rows);
		break;

	case 6:
		$res = ps_GetTopDownloads($rows);
		break;

	case 7:
		$res = ps_GetTopPosters($rows);
		break;

	case 8:
		$res = ps_GetTopReferrers($rows);
		break;

	case 9:
		$res = ps_GetTopThreadPosters($rows);
		break;
...
function ps_GetUnviewable($name = "")
{
	...
	if ($mybb->settings['ps_ignoreforums'])
	{
		...
		if (in_array($mybb->settings['ps_ignoreforums'], array(-1,
			'all'
		)))
		{
...
function ps_GetHighlight($query_arr = array())
{
	...
	if (is_array($under_mod_forums_arr) && in_array($query_arr['fid'], $under_mod_forums_arr))
	{
